...
impl ProxyPool {
    ...
    pub fn load_hot_from_db(&self, db: &Database, limit: usize) {
        let proxies = db
            .get_hot_proxy_records(limit)
            .unwrap_or_default()
            .into_iter()
            .map(|(row, quality)| Self::from_db_parts(row, quality))
            .collect();
        self.replace_all(proxies);
        tracing::info!("Loaded {} hot proxies into pool", self.proxies.len());
    }

    pub fn from_db_parts(row: ProxyRow, quality: Option<ProxyQuality>) -> PoolProxy {
        let quality = quality.map(ProxyQualityInfo::from);
    ...
    pub fn replace_all(&self, proxies: Vec<PoolProxy>) {
        self.proxies.clear();
    ...
    pub fn count_by_status(&self, status: ProxyStatus) -> usize {
        self.proxies.iter().filter(|p| p.status == status).count()
    }

    pub fn valid_with_errors_ids(&self) -> Vec<String> {
        self.proxies
            .iter()
    ...
    pub fn invalid_retry_ids(&self, max_error_count: u32, limit: usize) -> Vec<String> {
        let mut candidates: Vec<(String, u32)> = self
            .proxies
            .iter()
            .filter(|p| p.status == ProxyStatus::Invalid && p.error_count < max_error_count)
            .map(|p| (p.id.clone(), p.error_count))
            .collect();

        candidates.sort_by_key(|(_, error_count)| *error_count);
    ...
    pub fn pick_random(&self, filter: &ProxyFilter, count: usize) -> Vec<PoolProxy> {
        ...
        candidates.sort_by_key(|proxy| proxy.local_port.is_none());
